...
public class OpenApiResourceKindProvider : IResourceKindProvider
{
    ...
    {
        _lazyDefinitions =
            new Lazy<Task<IDictionary<string, JsonSchema>>>(LoadDefinitions,
                LazyThreadSafetyMode.ExecutionAndPublication);
        _lazyApiVersionKindSchemas = new Lazy<Task<ApiVersionKindSchemasDictionary>>(LoadApiVersionKindSchemas,
            LazyThreadSafetyMode.ExecutionAndPublication);
        _logger = logger;
    }
    ...
    {
        using var stream =
            typeof(OpenApiResourceKindProvider).Assembly.GetManifestResourceStream(typeof(OpenApiResourceKindProvider),
                "swagger.json");
        if (stream == null)
        {
            _logger.LogError(
