...
public class ResourcePatcher : IResourcePatcher
{
    ...
    private static FormatException NewFormatException(Context context)
    {
        return new FormatException(
            $"{context.Kind.Kind}.{context.Kind.ApiVersion} {context.Path} type {context.ApplyToken?.Type} is incorrect for {context.Element?.MergeStrategy}");
    }
    ...
    private JsonPatchDocument MergeListOfPrimative(JsonPatchDocument patch, Context context)
    {
        ...
        foreach (var liveValue in liveArray)
        {
            ...
            var lastAppliedIndex = lastAppliedList.FindIndex(lastAppliedValue =>
                _tokenEqualityComparer.Equals(lastAppliedValue, liveValue));
    ...
    private JsonPatchDocument MergeListOfObject(JsonPatchDocument patch, Context context)
    {
        ...
        var applyItems = apply
            .Select((item, index) => (name: item[context.Element.MergeKey]?.Value<string>(), index, item)).ToArray();
        var liveItems = live
            .Select((item, index) => (name: item[context.Element.MergeKey]?.Value<string>(), index, item)).ToArray();
        var lastAppliedItems =
            context.LastAppliedToken
                ?.Select((item, index) => (name: item[context.Element.MergeKey]?.Value<string>(), index, item))
                ?.ToArray() ?? Array.Empty<(string name, int index, JToken item)>();
