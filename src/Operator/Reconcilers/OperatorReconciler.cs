...
using Kubernetes.Operator.Generators;
    ...
    where TResource : class, IKubernetesObject<V1ObjectMeta>
{
    ...
    /// <inheritdoc/>
    public async Task<ReconcileResult> ReconcileAsync(ReconcileParameters<TResource> parameters,
            ...
            foreach (var generatedResource in generatedResources.Resources)
            {
                ...
                else
                {
                    ...
                    if (patch.Operations.Any())
                    {
                        ...
                        catch (ReconcilerException ex) when (ex.Status.Reason == "Invalid" &&
                                                             (ex.Status.Details?.Causes?.Any() ?? false))
                        {
                            _logger.LogWarning(
    ...
    private async Task<JsonPatchDocument> CalculateJsonPatchDocumentAsync(IKubernetesObject<V1ObjectMeta> applyResource,
        IKubernetesObject<V1ObjectMeta> liveResource)
    {
    ...
    private bool TryNewReconcilerException(HttpOperationException innerException,
        out ReconcilerException reconcilerException)
    {
        try
        {
