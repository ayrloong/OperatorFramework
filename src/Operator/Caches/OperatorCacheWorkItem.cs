...
namespace Kubernetes.Operator.Caches;
...
public struct OperatorCacheWorkItem<TResource> : IEquatable<OperatorCacheWorkItem<TResource>>
    where TResource : class, IKubernetesObject<V1ObjectMeta>, new()
{
    ...
    /// <returns></returns>
    public OperatorCacheWorkItem<TResource> SetRelated(
        ImmutableDictionary<GroupKindNamespacedName, IKubernetesObject<V1ObjectMeta>> related)
    {
        return new OperatorCacheWorkItem<TResource>(Resource, related);
    }
    ...
    public bool Equals(OperatorCacheWorkItem<TResource> other)
    {
        return EqualityComparer<TResource>.Default.Equals(Resource, other.Resource) &&
               EqualityComparer<ImmutableDictionary<GroupKindNamespacedName, IKubernetesObject<V1ObjectMeta>>>.Default
