using k8s;
using k8s.Models;
using Kubernetes.Operator.Caches;
using Kubernetes.Operator.Reconcilers;
...
public static class KubernetesOperatorExtensions
{
    ...
    public static IServiceCollection AddOperator<TResource>(this IServiceCollection services,
        Action<OperatorServiceCollectionBuilder<TResource>> configure)
        where TResource : class, IKubernetesObject<V1ObjectMeta>, new()
    {
        if (services is null)
        {
    ...
    public static IServiceCollection RegisterOperatorResourceInformer<TOperatorResource, TRelatedResource>(
        this IServiceCollection services)
        where TRelatedResource : class, IKubernetesObject<V1ObjectMeta>, new()
    {
        return services
            .RegisterResourceInformer<TRelatedResource>()
            .AddTransient<IConfigureOptions<OperatorOptions>,
                ConfigureOperatorOptions<TOperatorResource, TRelatedResource>>();
    }
}